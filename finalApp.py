...
class App:
    def __init__(self, window, window_title, video_source=0):
        ...
        self.canvas = tkinter.Canvas(window, width = self.vid.width, height = self.vid.height,background='black')
        self.canvas.pack()
        self.window.configure(background='#949da1')
 
        # Button that lets the user take a snapshot
        self.btn_start=tkinter.Button(window, text="START SIGNING", width=18,height=2 ,command=self.start,font=('Microsoft JhengHei UI Light', 12,'bold'),state=tkinter.NORMAL,cursor='hand1', background='#3f4345',borderwidth=0,foreground='white')
        self.btn_stop=tkinter.Button(window,text="END",width=18,height=2,command=self.stop,font=('Microsoft JhengHei UI Light', 12,'bold'),cursor='hand1',background='#3f4345',borderwidth=0,foreground='white')
        self.btn_stop["state"] = tkinter.DISABLED

        self.result_label = tkinter.Label(window,text="RESULT: ",font=('Microsoft JhengHei UI Light', 12,'bold'),background='#949da1')
